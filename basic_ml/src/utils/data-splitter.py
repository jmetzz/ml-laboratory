...
def create_argument_parser():
    ...
    parser.add_argument("-p", "--path", help="The path to the input files", required=True, default=".", type=str)

    parser.add_argument("-o", "--output", help="The path to save the new input files", required=True, type=str)

    parser.add_argument("-i", "--train-percent", default=1.0, help="The percentage of training images", type=float)

    parser.add_argument(
        "-v", "--validation-split", default=0.1, help="The percentage of training used as validation set", type=float
    )

    parser.add_argument("-t", "--test-percent", default=1.0, help="The percentage of test images", type=float)
    return parser
...
def export(src_path, out_path, train=None, validation=None, test=None, ext="jpg"):
...
if __name__ == "__main__":
    ...
    train_sample, _ = load_data(
        f"{cmdline_args.path}/meta/train.json", split_ratio=cmdline_args.train_percent, labels=target_labels
    )

    valid_sample, train_sample = split_set(train_sample, cmdline_args.validation_split)

    test_sample, _ = load_data(
        f"{cmdline_args.path}/meta/test.json", split_ratio=cmdline_args.test_percent, labels=target_labels
    )
